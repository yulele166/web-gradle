...
(function( $ ){
	...
	var methods = {
		...
		imageWidget: function(){
		    ...
		    var row = ' <div class="panel-body panel-form"> ';
		    row+='<div class="form-horizontal form-bordered" role="form" data-parsley-validate="true" >';
		    row += ' <div class="form-group"> <span class="control-label col-md-4 col-sm-4" for="fullname">插入图片</span>';
		    row+='<div class="col-md-7 col-sm-6"> <a id="f_addimg" class="wup-col"></a>';
		    row+=' <input type="hidden" id="hidd_addimg" name="hidd_addimg" /> <div>';
		    row+=' <span style=" color:red">顶部 (图片尺寸最少 宽：高=1：1 例 400：400，400：800) 其他部位尺寸不限</span></div>';
		    row += '</div></div></div> </div> <div class="clear-b"></div>';
			return row;
		},
		appsWidget: function ()
		{
		    //Class for Widget Handling the upload of Files
		    var row = '  <div class="modal-body" style="min-height: 430px;">';
		    row += ' <div class="col-md-8"> <h5 class="m-l-5"> 待选择</h5>';
		    row += '  <div class="col-md-11" style="padding-left: 5px;">';
		    row += '     <div class="btn-group"> <input type="text" placeholder="软件名、包名" id="txt_priorname" class="form-control" /> </div>';
		    row += '     <div class="btn-group"> <button class="btn btn-success" id="btn_priorsearch" type="button">    查询</button> </div>';
		    row += '  </div>  <div class="clear-b"></div> <ul class="mb-oplist" id="mb_toApp_wait"> </ul></div>';
		    row += '<div class="col-md-4"> <h5 class="m-l-5"> 已选择</h5> <ul class="mb-oplist mb-sorder" id="mb_toApp_selEd">';
		    row += '  </ul>  </div> <div class="clear-b"> </div></div>';
		    return row;
		},
		...
		init : function( options )
		{
			...
			var menuItems = { 'fonteffects': true,
						   ...
						   'insert_img'	: { "modal": true,
											...
											"beforeLoad":function(){ 
											    $("#f_addimg").find(".wup-add").removeClass("wup-add-ed").html("");
											    $("#hidd_addimg").val('');
											    var p_id = "f_addimg";
											    var f_callBack = function (ret)
											    {
											        var rjson = ret;
											        $("#" + p_id).find(".wup-add").html("<img src='" + rjson.fileurl + "' />");
											        $("#hidd_addimg").val(rjson.fileurl);
											    };
											    var posturl = "/Upload/Upload.ashx?t=up_opic&picpath=hs_img";
											    mwebupher.c_SinSPicImgUploader(posturl, p_id, f_callBack);
											},
											"onSave": function(){
												methods.restoreSelection.apply(this);												
												if ($("#hidd_addimg").val())
												{
													if(navigator.userAgent.match(/MSIE/i)){
													    var imageStr = '<img src="' + $("#hidd_addimg").val() + '"/>'
														methods.restoreSelection.apply(this,[imageStr,'html'])
													}
													else{
													    document.execCommand('insertimage', false, $("#hidd_addimg").val());
													}
												}
												else{
						   ...
						   'insert_apps': {
						       ...
						       "tooltip": "添加Apps",
						       "modalHeader": "添加Apps",
						       "modalBody": methods.appsWidget.apply(this),
						       "beforeLoad": function ()
						       {
						           pagefuns.load_resli_prioropen();
						           pagefuns.p_toapp_bind();

						           $("#btn_priorsearch").off('click');
						           $("#btn_priorsearch").on('click', function ()
						           {
						               pagefuns.load_resli_prioropen();
						           });
						       },
						       ...
						       {
						           methods.restoreSelection.apply(this);
						           var resTable = ' <div class="tab-con">  <table> <tr>';
						           var icount = 0;
						           $('#mb_toApp_selEd').children('li').each(function (i, n)
						           {
						           
						               var c = $(n), cid = c.attr('data-appid'), cname = c.attr('data-name'), cpkg = c.attr('data-pkg'), cicon = c.attr('data-icon'), ctype = c.attr('data-ctype');
						               ctype =( ctype && ctype) == '27' ? 0 : 1;
						               resTable += ' <td> <a href="javascript:;" data-sid="' + cid + '" data-spkgname="' + cpkg + '" data-stitle="' + cname + '" data-ctype="' + ctype + '" class="tab-con-href cli-app">';
						                   resTable += '<div class="img"><img src="' + cicon + '" alt="' + cname + '" title="' + cname + '" style="display: block;">';
						                   resTable += '<i class="hoticon"></i></div><div class="info"><h4>' + cname + '</h4> <span class="tab-con-download" downurl="##">安装</span>';
						                   resTable += ' </div></a> </td>';
						                   icount = i + 1;
						                   if (icount % 2 == 0)
						                       resTable += ' </tr> <tr>';
						           ...
						           resTable += ' </tr> </table> </div>';
						           //alert('onSaveaaa');
						           if (navigator.userAgent.match(/MSIE/i))
						               methods.restoreSelection.apply(this, [resTable, 'html'])
						           else
						               document.execCommand('insertHTML', false, resTable);

						           $("#InsertApps").modal("hide");
						           $(this).data("editor").focus();
						       }
						   },
						   //'insert_apps': {
			...
			var menuGroups = {'texteffects' : ['bold', 'italics', 'underline', 'color'],
							  ...
							  'actions' : ['undo', 'redo'],
							  'insertoptions' : ['insert_link', 'unlink', 'insert_img', 'insert_table','insert_apps'],
			...
			var $this = $(this).hide();	       	
	       	...
	       	var editor  = $( "<div/>",{	class : "Editor-editor",
										css : {overflow: "auto",height:"500px"},
